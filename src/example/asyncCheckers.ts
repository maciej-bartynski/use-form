...
export const asyncValidators: AsyncCheckerMethods<ExampleFormValues> = {
    mail: async function (txt) {
        return await new Promise((res) => {
            setTimeout(() => {
                const mailAlreadyExists = mockExistingMails.find(
                    (item) => item.toLowerCase() === txt.trim().toLowerCase(),
                );
    ...
    name: async function (txt, form) {
        return await new Promise((res) => {
            setTimeout(() => {
                const nameAlreadyExists = mockExistingNames.find(
                    (item) => item.trim().toLowerCase() === txt.trim().toLowerCase(),
                );
                const nameMailIncompatibile =
                    form.values.mail.split('@')[0].indexOf(txt.trim().toLocaleLowerCase()) < 0;
